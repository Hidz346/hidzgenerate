...
3. Buka SQL Editor, jalankan isi `supabase-setup.sql` — bikin bucket `hidzgenerate` (public) sekaligus policy baca & upload publiknya. Aman di-run ulang kapan aja, gak bakal error walau bucket/policy-nya udah ada.
4. (Opsional) Storage → bucket `hidzgenerate` → isi **File size limit** sesuai kebutuhan (misal 2GB) — cara paling gampang buat jaga kuota dari abuse. Default dari `supabase-setup.sql` tanpa batas.
5. Deploy ke Vercel seperti biasa (static site, gak perlu build step).
...
supabase-setup.sql  bikin bucket & policy publik di Supabase (lihat Setup)
```
